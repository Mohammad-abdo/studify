...
const AddRole = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(t('pages.roles.success') || (isRTL ? 'تم إنشاء الدور بنجاح' : 'Role created successfully'));
      navigate('/roles');
    } catch (error) {
      toast.error(isRTL ? 'فشل إنشاء الدور' : 'Failed to create role');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-10 page-transition pb-20">
      <PageHeader
        ...
        breadcrumbs={[{ label: isRTL ? 'صلاحيات الوصول' : 'RBAC' }, { label: t('pages.roles.defineNewRole') }]}
        backPath="/roles"
      />
      ...
      <form onSubmit={handleSubmit} className="flex flex-col xl:flex-row gap-10 items-start">
        ...
        <div className="w-full xl:w-96 space-y-8 shrink-0 lg:sticky lg:top-28">
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><Save size={20} /> {t('pages.roles.initializeRegistry') || (isRTL ? 'تهيئة السجل' : 'Initialize Registry')}</>}
            </button>
            <button
              type="button"
            ...
            >
              {t('pages.editRole.abortMission') || (isRTL ? 'إلغاء المهمة' : 'Abort Mission')}
            </button>
          </div>
        </div>
      </form>
